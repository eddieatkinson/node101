...
$(document).ready(()=>{
	...
	function buildRow(stockInfo, isSavedList){
		...
		newRow += `<tr>`;
			...
			newRow += `<td>$ ${Number(stockInfo.Ask).toFixed(2)}</td>`;
			newRow += `<td>$ ${Number(stockInfo.Bid).toFixed(2)}</td>`;
			newRow += `<td class="bg-${classChange}">$ ${Number(stockInfo.Change).toFixed(2)}</td>`;
			if(!isSavedList){
